...
app.use(
  ...
  graphqlHTTP({
    schema: schema,
    rootValue: root,
    graphiql: true, // เปิดใช้งช้านหน้าน้ทดสอบ GraphiQL ผ่าผ่ นเบราว์เซว์ อร์
  }),
);
...
app.get("/api/v1/students/:id/full", (req, res) => {
  ...
  const student = students.find((s) => s.id === id);
  if (!student) {
    return res.status(404).json({ message: "ไม่พบม่ ข้อข้มูลมู นักศึกศึษา" });
  }
  const studentCourses = courses.filter((c) =>
    student.courseIds.includes(c.id),
  );
